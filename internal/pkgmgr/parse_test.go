...
func TestParseDNFCheckUpdate(t *testing.T) {
	...
	modprobe := find(t, packages, "nvidia-modprobe")
	if modprobe.New != "3:590.48.01-1.fc42" {
		t.Errorf("nvidia-modprobe new = %q, want the epoch kept", modprobe.New)
	}
}
...
func TestParseDNFCheckUpdateSkipsErrors(t *testing.T) {
	out := "Error: Cache-only enabled but no cache for 'openai-chatgpt'\n" +
...
func TestParseDNFHistory(t *testing.T) {
	...
	if first.Command != "dnf install yt6801-dkms" {
...
func TestParsePacmanPendingIgnored(t *testing.T) {
	...
	nvidia := find(t, packages, "nvidia-utils")
	if !nvidia.Ignored {
